...
class Game(object):
    def __init__(self, players, middle, deck, playernumber, main_player):
        ...
        for x in self.players:
            if x.cash < 1500:
                x.cash += 2000
        if self.main_player.cash < 1000:
            print("You have too little cash to play... ")
            exit()
    ...
    def game(self):
        ...
        if thing3 == "EXIT":
            ...
            self.removedmainplayer = self.main_player
            return thing3
        thing4 = self.turn4(self.deck, thing3)
        if thing4 == "EXIT":
            for x in self.players:
                x.tempbetstate = True
                x.tempbetvalue = 0
